...
void handleDown_thenAll()
{
    if( gl_in[0].gl_Position.z/size == 2 && gl_in[1].gl_Position.z/size == 2 )
    {
             vec4 pos = gl_in[0].gl_Position + vec4(0,0,tesselatedSize,0);
            merge(pos);
              pos = gl_in[1].gl_Position + vec4(0,0,tesselatedSize,0);
            merge(pos);
            merge(gl_in[0].gl_Position);
            merge(gl_in[1].gl_Position);
            merge(gl_in[2].gl_Position);
    } 
    else if( gl_in[1].gl_Position.z/size == 2 && gl_in[2].gl_Position.z/size == 2 )
    {
             vec4 pos = gl_in[1].gl_Position + vec4(0,0,tesselatedSize,0);
            merge(pos);
            pos = gl_in[2].gl_Position + vec4(0,0,tesselatedSize,0);
            merge(pos);
            merge(gl_in[1].gl_Position);
            merge(gl_in[2].gl_Position);
            merge(gl_in[0].gl_Position);
    } 
    else if( gl_in[0].gl_Position.z/size == 2 && gl_in[2].gl_Position.z/size == 2 )
    {
             vec4 pos = gl_in[2].gl_Position + vec4(0,0,tesselatedSize,0);
            merge(pos);
            pos = gl_in[0].gl_Position + vec4(0,0,tesselatedSize,0);
            merge(pos);
            merge(gl_in[2].gl_Position);
            merge(gl_in[0].gl_Position);
            merge(gl_in[1].gl_Position);
...
void handleSides()
{
    ...
    for(int i=0; i<3; i++)
    {
        ...
        if(gl_in[i].gl_Position.x/size == 2)
        {
            cntRight++;
        }

        else if(gl_in[i].gl_Position.x/size == -2)
        {
            cntLeft++;
        }
    }
        ...
        if(cntLeft >= 2)
        {
            vec4 pos = gl_in[0].gl_Position + vec4(-tesselatedSize,0,0,0);
            merge(pos);
            pos = gl_in[1].gl_Position + vec4(-tesselatedSize,0,0,0);
            merge(pos);
            merge(gl_in[0].gl_Position);
            merge(gl_in[1].gl_Position);
            merge(gl_in[2].gl_Position);

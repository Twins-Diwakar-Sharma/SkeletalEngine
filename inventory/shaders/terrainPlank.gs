...
out vec2 fragAbsAdjustedObjectPos;
out float doMode;
...
void createVertex(int i)
{
    ...
    fragAbsAdjustedObjectPos = geoIn[i].fragAbsAdjustedObjectPos;
    doMode = geoIn[i].doMode;
    
    EmitVertex();
}

void createNewVertex(vec3 gWorldPos, vec3 gFragWorldPos, vec2 gFragAbsAdjustedObjectPos, float gDoMode)
{
    vec3 worldPos = gWorldPos;
    worldPos.y = getHeightFromTexture(worldPos.x,worldPos.z);
    fragWorldPos = gFragWorldPos;
    fragAbsAdjustedObjectPos = gFragAbsAdjustedObjectPos;
    doMode = gDoMode;
...
void main()
{
    ...
    vec2 newFragAbsAdjustedObjectPos = geoIn[rightAngleIndex].fragAbsAdjustedObjectPos;
    float newDoMode = geoIn[rightAngleIndex].doMode;

    createVertex(otherIndexA);
    createVertex(rightAngleIndex);
    createNewVertex(newWorldPos,newFragWorldPos,newFragAbsAdjustedObjectPos,newDoMode);
